...
- **Plugins:** `manifest.json` (UUID, permissions, i18n, UI templates) + server `main.js` (default-exports `definePlugin()` from `@hoardodile/plugin-sdk-server`) + sandboxed iframe client (`@hoardodile/plugin-sdk-web`, or `plugin-sdk-react` for React). Third-party plugins join the dev loop via `DEV_PLUGINS` paths — see `plugins/template`.
