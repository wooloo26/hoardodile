...
- **Plugins:** `manifest.json` (UUID, permissions, i18n, UI templates) + server `main.js` (default-exports `definePlugin()` from `@hoardodile/plugin-sdk-server`) + sandboxed iframe client (`@hoardodile/plugin-sdk-web`, or `plugin-sdk-react` for React). Server hooks run in a per-plugin resident worker thread (`apps/server/src/domain/plugin/sandbox/`): the `ResourceAPI` is bridged over structured-clone RPC, an activity watchdog kills hung workers (lazily respawned, spawn-rate limited), and `resourceLimits` caps worker memory — so a spinning/crashing plugin never blocks the main event loop. Third-party plugins join the dev loop via `DEV_PLUGINS` paths — see `plugins/template`.
