...
- CI: `.github/workflows/ci.yml` runs `pnpm lint`, `pnpm test`, `pnpm licenses:check`, `pnpm version:check`, and `pnpm build` in the `check` job, plus a Playwright `e2e` job — all on an ubuntu + windows matrix, on push/PR to `main`. Releases are cut locally with `pnpm release` only (no CI release workflow). Dependabot (`.github/dependabot.yml`) opens weekly grouped PRs for npm and GitHub Actions updates. The docs site is deployed to GitHub Pages by `.github/workflows/docs.yml` on pushes to `main` that touch `apps/docs/` (requires repo Settings → Pages → Source = "GitHub Actions"; served at the custom domain `docs.hoardodile.com` via `apps/docs/public/CNAME`).
